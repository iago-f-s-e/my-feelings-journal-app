...
export const CreateHappeningDiaryModal = ({
  visible,
  onClose,
  onSave,
}: SelfCareModalProp) => {
  const happeningDiary = useUIStore(state => state.happeningDiary);
  ...
  const memTitle = useMemo(() => {
    return title || happeningDiary.title;
    ...
    <Modal visible={visible} transparent animationType="fade">
      <KeyboardAvoidingView
        ...
        <View style={styles.content}>
          ...
          <TouchableOpacity
            disabled={!memTitle?.trim().length}
            style={!memTitle?.trim().length ? styles.saveDisabled : styles.save}
            onPress={() => {
              onSave(
                memTitle as string,
                description as string,
                feelingType as FeelingType,
                happeningDiary.id?.toString(),
              );
